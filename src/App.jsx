...
export default function App() {
  ...
  const aspects = data.length
    ? Object.keys(data[0]).filter(
        (c) => typeof data[0][c] === 'number' && !base.includes(c)
      )
    : [];

  useEffect(() => {
    if (aspects.length && !keyword) {
      setKeyword(aspects[0]);
    }
  }, [aspects, keyword]);
  ...
  const sentimentColor =
    review.sentiment_score > 0.2
      ? 'text-green-600'
      : review.sentiment_score < -0.2
      ? 'text-red-600'
      : 'text-gray-600';

  const summaryIntBubble = aspects
    .map((a) => {
      ...
      const mean = sum / mentions;
      const variance =
        scores.reduce((acc, s) => acc + (s - mean) ** 2, 0) / mentions;
      const stdDev = Math.sqrt(variance);

      return {
        keyword: a,
        mentions,
        avg_sent: Number((mean * 100).toFixed(3)),
        std_sent: Number((stdDev * 100).toFixed(3)),
      };
    })
  ...
  const avg =
    valid.length > 0
      ? (valid.reduce((s, r) => s + r[keyword], 0) / valid.length).toFixed(2)
      : 'N/A';
  ...
  aspects.forEach((a) => {
    data.forEach((r) => {
      ...
      if (v !== null && v !== undefined && !isNaN(v)) {
        reviewsDfLong.push({ keyword: a, sentiment: v });
      }
    });
  });
    ...
    >
      <header className="text-center py-8">
        <h1 className="text-5xl font-extrabold" style={{ color: 'white' }}>
          Sentiment Dashboard: Pho Rowland
        </h1>
      </header>
      ...
      <div className="w-full overflow-hidden leading-none rotate-180">
        ...
        >
          <path
            d="M0,0 C1280,100 1280,100 2560,0 L2560,100 L0,100 Z"
            fill="white"
          />
        </svg>
      </div>
      ...
      <motion.div
        ...
        className="bg-gray-50 rounded-2xl shadow-lg p-6 mx-8 mt-8 mb-12"
      >
        ...
        <p className="text-sm">
          ...
          <span className={sentimentColor}>{review.sentiment_score}</span>
        </p>
        ...
        >
          <div className="mb-4 flex justify-between items-center">
            ...
            <select
              ...
              className="px-3 py-2 rounded-lg bg-white shadow hover:shadow-md transition"
            >
              {aspects.map((a) => (
      ...
      <div className="px-8 grid md:grid-cols-2 gap-8 mb-12">
        ...
        >
          ...
          <img
            src="/sentiment_dist.png"
            alt="Sentiment Score Distribution"
            className="w-full rounded"
            style={{ maxHeight: 400, objectFit: 'contain' }}
          />
          <ul className="mt-2 ml-4 list-disc text-gray-700">
      ...
      <div className="px-8 grid md:grid-cols-2 gap-8 pb-12">
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
          className="bg-gray-50 rounded-2xl shadow-lg p-6 flex"
        >
          <div className="w-2/3">
            <h2 className="text-2xl font-semibold mb-4">
              Mentions vs. Average Sentiment
            </h2>
            <PlotlyBubble summaryData={summaryIntBubble} />
          </div>
          <div className="w-1/3 pl-8 flex flex-col justify-center space-y-2">
            <p className="text-gray-600">
              High mentions + High sentiment: Frequently mentioned & viewed very positively
            </p>
            <p className="text-gray-600">
              High mentions + Low sentiment: Frequently mentioned but often criticized
            </p>
            <p className="text-gray-600">
              Low mentions + High sentiment: Niche but highly praised
            </p>
            <p className="text-gray-600">
              Low mentions + Low sentiment: Niche and viewed negatively
            </p>
          </div>
        </motion.div>
