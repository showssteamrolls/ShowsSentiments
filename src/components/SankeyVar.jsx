...
export default function SankeyVar({
    rows,
    csvUrl="/cleaned_sentiment_keywords.csv",
    topN=15,
    twoStep=false,
    height=520,
    title="Keywords → Variability"
}) {
    ...
    const { nodes, links } = useMemo(()=> {
        ...
        // --- FORMAT 1: summary_df (keyword,count,mean,std,std_bucket[,mean_bucket])
        const looksLikeSummary = ["keyword","count","std"].every(k => cols.includes(k))
        if (looksLikeSummary) {
            const s = [...data]
                .map(r => ({ ...r, count:Number(r.count||0), std:Number(r.std||0), mean:Number(r.mean||0) }))
                .filter(r => r.keyword && r.count>0)
                .sort((a,b)=>b.count-a.count)
                .slice(0, topN)

            const hasMeanBucket = s.some(r => r.mean_bucket)
            if (!twoStep || !hasMeanBucket) {
                const left=s.map(r=>r.keyword)
                const right=[...new Set(s.map(r=>r.std_bucket))]
                const names=[...left, ...right]; const idx=new Map(names.map((n,i)=>[n,i]))
                return {
        ...
        const stats = keywordCols.map(col => {
            ...
            for (const r of data) {
                const v=r[col]
                if (typeof v==="number" && !Number.isNaN(v)) vals.push(v) // 0 is valid
            }
        ...
        if (!twoStep) {
            ...
            Object.entries(agg).forEach(([k,v])=>{ const [s1,m1]=k.split("||"); links.push({ source:idx.get(s1), target:idx.get(m1), value:v }) })
            return { nodes:names.map(n=>({ name:n })), links }
        }
    }, [table, topN, twoStep])
